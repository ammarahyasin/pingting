...
class Proxy(NetworkApplication):
    ...
    def __init__(self, args):
        ...
        if __name__ == "__main__":
            ...
            serverSocket = socket.socket(socket.AF_INET, socket.SOCK_STREAM) #connectionless socket: used to establish a TCP connection with the HTTP server.
            serverSocket.listen(5)
    ...
    def handleRequest(clientSocket):
        ...
        # 1. Receive request message from the client on connection socket
        bufferSize = clientSocket.CMSG_SPACE(4) #IPv4 address is 4 bytes in length
    ...
    def localObject(requestMessage): #proxy server checks to see if copy of object is stored locally
